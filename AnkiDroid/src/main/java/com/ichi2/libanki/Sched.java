...
public class Sched {
    ...
    private void _rescheduleAsRev(Card card, JSONObject conf, boolean early) {
        ...
        if (lapse) {
            card.setIvl(_adjRevIvl(card, card.getIvl()));
        ...
        if (mLoadBalancer) {
            ...
            try {
                ...
                for (int i = ivl_min; i <= ivl_max; i++) {
                    ...
                    int siblings = getCol().getDb().queryScalar(s, false);
                    s = String.format("select factor from cards where due = %d and queue = 2", due);
                    List<Integer> cards = getCol().getDb().queryColumn(Integer.class, s, 0);
                ...
                Log.d(AnkiDroidApp.TAG, String.format("load balancer: %d -> %d", idealIvl, lowest.get(0)));
    ...
    public void setLoadBalancer(boolean mLoadBalancer) {
